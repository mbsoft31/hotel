            ...
            <div class="bg-white max-w-5xl mx-auto px-8 py-16 space-y-4 rounded-md shadow">
                <div class="max-w-3xl mx-auto space-y-4">
                    <div class="w-full space-y-4">
                        <x-input.label for="search" value="{{ __('Search for Hotel name') }}" class="text-xl tracking-wide" />
                    ...
                    {{--<div class="w-full space-y-4">
                        <x-input.label for="country" value="{{ __('Country') }}" class="text-xl tracking-wide" />
                        <x-input.text wire:model.debounce.300="country" name="country" id="country" placeholder="Search for name, address then just hit enter" class="w-full text-xl border border-gray-300 focus:border-purple-300 outline-none focus:ring-offset-1 focus:ring-2 focus:ring-purple-500"/>
                    </div>--}}
                    <div class="w-full space-y-4">
                        <x-input.label for="city" value="{{ __('City') }}" class="text-xl tracking-wide" />
                        <x-input.text wire:model.debounce.300="city" name="city" id="city" placeholder="Search for name, address then just hit enter" class="w-full text-xl border border-gray-300 focus:border-purple-300 outline-none focus:ring-offset-1 focus:ring-2 focus:ring-purple-500"/>
                    </div>
                                ...
                                <div class="col-span-full md:col-span-10 whitespace-normal">
                                    <div class="flex items-center text-lg font-semibold tracking-wider text-gray-800">
                                        <div class="flex-grow space-y-2">
                                            ...
                                            <a href="{{ route("hotel.show", ["hotel"=>$hotel]) }}" class="pb-8 block text-purple-600 hover:text-purple-700">{{ $hotel->name }}</a>
                                        </div>
                                        <div class="space-y-2">
                                            <div>
                                                <a href="{{ route("hotel.show", ["hotel"=>$hotel]) }}" class="inline-block px-2 py-2 font-semibold tracking-wider whitespace-nowrap text-purple-600 rounded-md border border-transparent hover:text-purple-700 hover:border-purple-700">
